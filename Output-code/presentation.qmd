...
* from `r min(sp500$date.ts)` to `r max(sp500$date.ts)`
* `r length(sp500$date.ts)` observations
* If you like thousand commas in your numbers, then  `r prettyNum(length(sp500$date.ts), big.mark = ",")` observations
